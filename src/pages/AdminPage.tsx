...
import {
  ...
  type UserDTO, type ParcelDTO, type BranchDTO, type SubscriptionRequestDTO,
} from '../api';
...
import {
  ...
  UserPlus, AlertCircle, RefreshCw, MapPin, Plus, Edit3, Save, X, Loader, Star
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    { key: 'payments', label: 'Оплата',    icon: <CreditCard size={14} />, badge: paymentRequests.length + subRequests.length },
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {paymentRequests.length === 0 ? (
                    <div className="text-center py-6 glass-card-static">
                      <CheckCircle size={28} className="text-dark-400 mx-auto mb-2" />
                      <p className="text-sm text-dark-400">Нет заявок на оплату посылок</p>
                    </div>
                    ...
                    <div key={parcel.id} className="glass-card-static p-4 space-y-3">
                      ...
                      <button onClick={() => handleConfirmPayment(parcel.id)} className="btn-primary w-full text-sm"><CheckCircle size={14} />Подтвердить оплату посылки</button>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {subRequests.length === 0 ? (
                    <div className="text-center py-6 glass-card-static">
                      <CheckCircle size={28} className="text-dark-400 mx-auto mb-2" />
                      <p className="text-sm text-dark-400">Нет заявок на подписку</p>
                    </div>
                  ) : subRequests.map(req => (
                    <div key={req.id} className="glass-card-static p-4 space-y-3">
                      <div className="flex items-center gap-2">
                        <Star size={14} className="text-yellow-400" />
                      ...
                      <div className="flex items-center justify-between bg-yellow-500/10 rounded-lg p-3 border border-yellow-500/20">
                        <div><p className="text-xs text-dark-400">Сумма</p><p className="text-lg font-bold text-yellow-400">{req.amount} кбк</p></div>
                        <div className="flex items-center gap-1 text-xs text-yellow-400"><AlertCircle size={14} />Ожидает проверки</div>
                      </div>
                      <p className="text-xs text-dark-400 text-center">
                        {new Date(req.createdAt).toLocaleString('ru', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })}
                      </p>
                      <p className="text-xs text-dark-400">Проверьте поступление <span className="text-yellow-400 font-semibold">5 кбк</span> на счёт <span className="text-red-400 font-semibold">kbpost</span> от пользователя <span className="text-white font-semibold">{req.username}</span>.</p>
                      <div className="flex gap-2">
                        <button
                          onClick={async () => {
                            try {
                              await apiRejectSubscription(req.id);
                              setSubRequests(prev => prev.filter(r => r.id !== req.id));
                              addToast('Запрос отклонён', 'info');
                        ...
                        <button
                          onClick={async () => {
                            try {
                              await apiConfirmSubscription(req.id);
                              setSubRequests(prev => prev.filter(r => r.id !== req.id));
                              addToast(`Подписка для ${req.username} активирована на 1 месяц ✅`, 'success');
                            } catch(e: any) { addToast(e.message || 'Ошибка', 'error'); }
                          }}
                          className="btn-primary flex-1 text-sm"
                        >
                          <CheckCircle size={14} />Подтвердить подписку
                        </button>
                      </div>
                    </div>
                  ))}
